...
export default function StudentManagement() {
  ...
  const loadStudents = async (withRefreshState = false) => {
    if (withRefreshState) {
      setRefreshing(true);
    } else {
      setLoading(true);
    ...
    try {
      const res = await fetch("/api/registrar/students", { cache: "no-store" });
      if (!res.ok) {
  ...
  const saveEdit = async () => {
    ...
    try {
      const res = await fetch(`/api/registrar/students/${editingId}`, {
        method: "PATCH",
