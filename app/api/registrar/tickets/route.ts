...
export async function GET() {
  ...
  const { data, error } = await admin
    .from("tickets")
    .select(
      "id, query, status, priority, category, confidence_score, conversation_id, user_id, assigned_to, created_at, updated_at, resolved_at, users!tickets_user_id_fkey(name, email, roll_number, course_code, school_code)"
    )
  ...
  const tickets = (data ?? []).map((ticket) => {
    const user = ticket.users as unknown as { name: string; email: string; roll_number: string | null; course_code: string | null; school_code: string | null } | null;
    return {
      id: ticket.id,
      query: ticket.query,
      ...
      student_course: user?.course_code ?? null,
      student_school: user?.school_code ?? null,
    };
  });
